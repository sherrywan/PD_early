...
LastEditTime: 2024-11-12 10:30:20
...
if __name__ == "__main__":
    ...
    cross_val_nums=10
    best_crossval_time_t=np.load(os.path.join(save_dir, "MLP_best_idx.npy"))
    ...
    if not os.path.exists(os.path.join(save_dir, "modalily_shapley_ratio.yaml")):
        ...
        for idx, model_t in enumerate(model_types):
            ...
            for cross_idx in range(cross_val_nums):
                if cross_idx > 0:
                    ...
                    axs[idx].barh(vis_index, test_datas[f'fold-{cross_idx}'], height=bar_width, left=left_loc, label=f'fold-{cross_idx}', color=cross_colors[cross_idx])
                else:
                    axs[idx].barh(vis_index, test_datas[f'fold-{cross_idx}'], height=bar_width, label=f'fold-{cross_idx}',color=cross_colors[cross_idx])
            if idx == 0:
                axs[idx].set_yticks(y_pos, np.array(modality_shows), fontsize=32)
            else:
                axs[idx].set_yticks([])
            # axs[idx].set_xticks(range(0,3.0,0.5),fontsize=20)
            axs[idx].set_title(model_t, fontsize=32)
        ...
        plt.axvline(0.36,linestyle="--", color="black", alpha=0.8)
        plt.xticks(fontsize=16)
        plt.yticks(y_pos, np.array(modality_shows), fontsize=24)
